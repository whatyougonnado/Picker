...
int main(int argc, char *argv[])
{
	...
	for (auto i = 0; i < obj_list.size() && isFront; ++i) {
		...
		for (auto k = 0; k < texmesh_name_list.size(); ++k) {
			...
			std::string save_path = image_path + mg::convertExtension("bd_" + texmesh_name_list[k], "jpg"); // <- need to convert jpg
			...
			save_path = image_path + "bd_" + id_name_list[k];
	...
	for (auto i = 0; i < obj_list.size() && !isFront; ++i) {
		...
		for (int k = 0; k < 36; ++k) {
			...
			std::string bound_id_full_path = bound_id_name_list[k];
