...
map<string, array<float, 3>> PictureComparison::getColorTableDouble() const
{
	map<string, array<float, 3>> ret;

	for (auto iter : color_table_) {
		string name = iter.first;
		array<float, 3 > rgb = array<float, 3>{(float)((int)rgb[0]), (float)((int)rgb[1]), (float)((int)rgb[2])};
		
		ret.insert(pair<string, array<float, 3>>{name, rgb});
	}

	return map<string, array<float, 3>>();
}
